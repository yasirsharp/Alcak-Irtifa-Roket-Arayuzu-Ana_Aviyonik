...
namespace Hafız_IHO_Steam_Alcak_Irtif
{
    public partial class Form1 : Form
    {
        ...
        private void seriP1_DataReceived(object sender, SerialDataReceivedEventArgs e) {
            dataIn = seriP1.ReadExisting(); //Örnek gelen veri - A1550B40,8401C31,1512FKUX
            ...
            index_of_sensor_err = Convert.ToSByte(dataIn.IndexOf('U'));
            index_of_gps_err = Convert.ToSByte(dataIn.IndexOf('X'));
            #endregion
        ...
        private void DataProcessing() {
			...
			try{
				...
				if (lblenlem.Text != "Enlem" && lblboylam.Text != "Boylam") {
                    ...
                    label4.Text = map.Position.ToString();
				}
			}
			catch(Exception err){
            ...
            #region sensor error
            if (index_of_sensor_err >= 0)
            {
                btnSenErr.BackColor = Color.DarkOrange;
                btnSenErr.Text = "Sensor Hatası";
            }
            #endregion
        }
        #endregion
        ...
        private void LogData() {
            try {
				...
				File.AppendAllText(errLogFilePath, $"Log Hatası {err.Message}  |  {DateTime.Now} \n");
            }
        }
        #endregion
